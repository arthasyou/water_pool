...
-export([draw/3]).
-export([mixture_status/2, mixture_data/1]).
...
mixture_status(State, Data) ->
    ...
    #{
        state => State,
        pot => Pot / Ratio,
        wave => Data#mixture_data.wave,
        segment => Data#mixture_data.segment,
        suction => Suction div Ratio,
        bonus => Bonus / Ratio,
        brokerage => Suction * BrokerageRatio div Ratio / Ratio,
        big_bonus => mixture_dict:get_big_bonus(),
        bb_line => mixture_dict:get_ascent_big_bonus_base_line(),
        miss_count => mixture_dict:get_miss_count()
    }.
...
mixture_data(Data) ->
    ...
    {Head, _} = Data#mixture_data.segment,
    Wave = lists:map(fun(X) ->
        X / Ratio
    end, [Head|Data#mixture_data.wave]),
    #{
        pot => Pot / Ratio,
        wave => Wave,
        suction => Suction div Ratio,
